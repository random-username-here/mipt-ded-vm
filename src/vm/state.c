...
static bool _read_cpureg(
    ...
    if (off < 8) // null "page"
        ...
        *out = index_num_const(state->interrupt_vector[off / 8 - 1], off % 8);
    else {
        off -= 0x100;
        switch(off) {
        case 0:
            *out = state->exception_type; break;
        case 0x1 ... 0x8:
            *out = index_num_const(state->exception_pc, off - 0x1); break;
        case 0x9 ... 0x11:
            *out = index_num_const(state->exception_segfault_addr, off - 0x9); break;
        default:
            return vm_mem_segfault(state, dev->base + off);
        }
    }
...
static bool _write_cpureg(
    ...
    if (off < 8 || off >= 0x100)
        return vm_mem_segfault(state, dev->base + off);
    *index_num(&state->interrupt_vector[off / 8 - 1], off % 8) = val;
    return true;
}
...
void vm_state_init(vm_state *state, vm_log_fn logger)
{
    ...
    *state = (vm_state) {
        ...
        .rom = NULL,
        .rom_size = 0,
        .stack = ia_new_empty_array$(vm_stack_val_t),
        .is_halted = false,
        .should_die = false,
        .interrupts_disabled = false,
        // .interrupt_vector zeroed
        .interrupt_type = ia_new_empty_array$(vm_interrupt_type),
        .interrupt_return_addr = ia_new_empty_array$(vm_stack_val_t), // TODO: remove that
        .asked_interrupts = ia_new_empty_array$(vm_interrupt),
        .exception_pc = 0,
        .exception_type = VM_EXC_NONE,
        .log_fn = logger,
        .v2_sp = 0,
        .v2_sf = 0,
        .devices = ia_new_empty_array$(vm_dev*),
        .mmreg = (vm_dev) {
            .type = VM_DEV_CPUREGS,
            .name = "CPU memory-mapped registers",
            .size = 0x120,
            .reader = _read_cpureg,
            .writer = _write_cpureg
        }
    };

    check$(state->ram, "VM needs %d bytes of RAM", RAM_SIZE);
    vm_state_mount_dev(state, &state->mmreg, 0x0);
}
